...
|---|---|---|
| Disposition vocabulary | `success` / `rejected` / `failure` / `timeout` | plus **`cancelled`** — a subscription the caller abandoned is the reactive reality a blocking call cannot have; a consumer that stops reading the body because it has read enough is `success`, partially read |

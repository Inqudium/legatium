 ...
 * instance's open-exchange movements become invisible. Every interceptor therefore obtains its metrics
 * through [forRegistry], and interceptors on one registry share one owner - the gauge then reports the
 ...
 * working and the affected meter is simply not exported (twin parity with the WebClient module).
 */
internal class ClientLoggingMetrics private constructor(
    ...
    // The liveness check of the emission architecture itself: since the event is emitted at response
    // close, everything rests on the client closing every response the interceptor handed out. A
    // response that is never closed loses the event SILENTLY - nothing throws, so not even the
    // fail-open counter sees it. This gauge (up at interceptor entry, down at completion) makes the
     ...
     * INCLUDING bodiless answers the application never touched - that is exactly the `unread` share the
    ...
    companion object {
        ...
        // VALUE is exactly what every interceptor already holds strongly - the owner lives as long as an
        // interceptor using it does. Residual (accepted): when every interceptor of a still-live registry
        // has been collected and a NEW one is wired against it afterwards, the fresh owner meets its own
         ...
         * interceptors run against one registry: a duplicate owner's gauge registration would be silently
         * ignored (see the class documentation), a shared owner makes the gauge the total across its
         * interceptors while the counters merge as before.
         */
        ...
        /**
         * Gauge of exchanges between interceptor entry and response close. Hovers near the in-flight
         * call count in health; a monotonically growing baseline means responses are not being closed
        ...
        /** The closed request-id source vocabulary of [CORRELATION_METER] - shared with the interceptor. */

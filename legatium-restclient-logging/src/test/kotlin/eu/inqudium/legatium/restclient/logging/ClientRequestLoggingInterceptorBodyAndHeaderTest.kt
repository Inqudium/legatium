...
class ClientRequestLoggingInterceptorBodyAndHeaderTest {
    ...
    inner class `Request body` {
        ...
        fun `should decode the request body with the declared charset`() {
            ...
            // Then
            assertThat(keyValues(log.events.single())).containsEntry("adapter_request_body", "café")
        }
    ...
    inner class `Outcome-gated bodies` {
        ...
        fun `should log a decoding failure of the application as a successful exchange without bodies in on-failure mode`() {
            ...
            //   the module observes the wire, not the application's decoding, and there is no seam
            //   through which a converter failure could reach the interceptor. Pinned so a change here

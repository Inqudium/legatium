...
class ClientRequestLoggingInterceptorTest {
    ...
    inner class `Levels and outcomes` {
        ...
        fun `should log INFO with outcome rejected for a 4xx answer`() {
            ...
            //   exception is `rejected` at INFO (ADR-0012).
            // Success criteria: one INFO event with "-> 404" in the message, adapter_outcome rejected and
            //   status field 404.
            // Why it matters: a 404 on a lookup is a completed call the application handles; the outcome
            //   lets a dashboard split it from a success without raising the severity.
            // Given/When: the peer answers 404
            interceptor.intercept(request(), ByteArray(0), answering(status = HttpStatus.NOT_FOUND)).consumeAndClose()

            // Then
            val event = log.events.single()
            assertThat(event.level).isEqualTo(Level.INFO)
            assertThat(event.formattedMessage).contains("-> 404 [")
        ...
        fun `should escalate a rejected answer to WARN for 401, 403, 408 and 429`() {
            ...
            // Given/When: the peer refuses us four ways
            for (status in listOf(HttpStatus.UNAUTHORIZED, HttpStatus.FORBIDDEN, HttpStatus.REQUEST_TIMEOUT, HttpStatus.TOO_MANY_REQUESTS)) {
                interceptor.intercept(request(), ByteArray(0), answering(status = status)).consumeAndClose()
            }

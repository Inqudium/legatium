 ...
 * blocking twin's tee) or in chunks of {@code chunk} bytes (8 KiB, the bulk-read shape). The loop
 * is the operation - one body - not amplification of a smaller op.
 *
...
public class BodyBufferWriteBenchmark {
    ...
    /**
     * Body size; the 16 KiB point is the shipped default cap, 256 KiB a cap a debugging profile
     * might set.
     */
    @Param({"1", "16", "256"})
    public int bodyKb;

    /** 1 = byte-wise writes through {@code write(int)}; any other value is the bulk write width. */
    @Param({"1", "8192"})
    public int chunk;
    ...
    @Benchmark
    public ByteArrayOutputStream byteArrayOutputStreamPresized() throws IOException {
    ...
    private void writeBounded(BoundedByteBuffer buffer) {
        if (chunk == 1) {
            ...
            for (int offset = 0; offset < body.length; offset += chunk) {
                buffer.write(body, offset, Math.min(chunk, body.length - offset));
            }
        }
    }

    /**
     * One helper for both streams: a JMH fork runs one benchmark method, so the call stays
     * monomorphic.
     */
    private void writeStream(OutputStream stream) throws IOException {
        if (chunk == 1) {
            ...
            for (int offset = 0; offset < body.length; offset += chunk) {
                stream.write(body, offset, Math.min(chunk, body.length - offset));
            }
        }
    }
}

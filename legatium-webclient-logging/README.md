...
|---|---|---|
| Disposition vocabulary | `success` / `rejected` / `failure` / `timeout` | plus **`cancelled`** — a subscription the *caller* abandoned (a downstream `timeout()` operator, a disposed caller, a client disconnecting mid-stream) is the reactive reality a blocking call cannot have. A consumer that stops reading the body from within its own delivery because it has read enough — Spring's body skip for `bodyToMono(Void.class)`, a `take(n)` — is `success` with the body partially read, not `cancelled`. Note: a `Mono.timeout()` the *caller* applies logs `cancelled`; a timeout the *connector* raises (Reactor Netty's `responseTimeout`) logs `timeout` |
...
| WARN breadcrumb on a thrown call | a short WARN line on the module's own logger the moment the call throws, before the ERROR event | **none** — a failed call is reported through the exchange event alone (the error is a signal that reaches the terminal callback anyway); only an `Error` escaping the assembly leaves a breadcrumb, as in the twin |
| Retry identity on a traceless call | the mutable request keeps the correlation header of attempt 1, so every re-entry of a retrying outer interceptor sends the same generated id | the identity is resolved per subscription from the caller's immutable `ClientRequest`, which never carries the header the filter adds to its rebuilt copy — every attempt of a resubscribed traceless call generates and sends a **fresh** id. A traced call carries the same trace id on every attempt on both stacks |
...
Everything whose twin copies genuinely differ (emitters, exchanges, filter vs. interceptor, body capture

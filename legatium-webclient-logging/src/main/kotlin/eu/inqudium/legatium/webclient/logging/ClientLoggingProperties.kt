...
/**
 * Configuration surface of the client-logging WebClient filter, bound from the `client-logging.*`
 * namespace - the keys and defaults are identical to legatium-restclient-logging's, key for key and
 * default for default. `ClientLoggingReferenceConfigTest` enforces exactly that contract by binding the
 * shared reference YAML against THIS class.
 *
...
data class ClientLoggingProperties(
    /** Master switch; `false` removes the filter and its customizer entirely (auto-configuration backs off). */
     ...
     * is supported as well) that determine for which request PATHS the filter is active AT ALL,
     ...
     * header sections' rule. Invalid patterns fail the context start (parsed once at filter
     * construction).
     */
    val includePathPatterns: List<String> = emptyList(),
    /**
     * Request-path prefixes that are not logged at all (the filter does not even wire an exchange
     ...
     * and disappear with a logging flag. Measure-only installs a count-only tee - nothing is
     * buffered. Recorded at the exchange's completion (see the emission point on
     * `ClientRequestLoggingFilter`), and only for bodies that actually flowed (zero bytes record no
     * sample).
     */
    ...
    companion object {
         ...
         * outgoing request; a connector that validates field names would reject a non-token at runtime
         * on EVERY call - failing the CALL, not merely the log line - so it is validated at binding time.
         */

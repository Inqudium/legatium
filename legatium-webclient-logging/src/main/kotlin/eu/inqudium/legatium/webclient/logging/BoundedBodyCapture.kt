 ...
 * mutation and every read runs under one uncontended [ReentrantLock]. Invariant: the emitter calls
 * [freeze] FIRST - from then on the capture is immutable, a late tee call is a no-op, and the logged
 * body and the size sample are one consistent snapshot instead of a moving target.
 *

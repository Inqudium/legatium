...
case-insensitive match on the URI's host without port — an IPv6 literal matches with or without its
brackets (`::1` and `[::1]` both exclude `http://[::1]:8080/`), and an authority `java.net.URI` refuses
to parse as a host, such as a service name with an underscore (`billing_api`), matches as written; the
same rule names the peer in `adapter_url_host` and the route. Path matching sees the target the way a server
